...
public class ProducStatusService {
	...
	public ProductStatus getRecordService(Long id) throws ObjectNotFoundException {
		...
		Product pr = restTemplate.getForObject(
		          "http://127.0.0.1:9002/GetProductInfo/1",
		          Product.class);
	...
	public ResponseEntity<String> productfallback(Exception e) {
		return new ResponseEntity<String>("ProductService not Found",HttpStatus.OK);
	}
	public ResponseEntity<String> productfallback1(Exception e) {
		return new ResponseEntity<String>("ProductService not Found",HttpStatus.OK);
	}
	public ResponseEntity<String> productfallback2(Exception e) {
		return new ResponseEntity<String>("ProductService not Found",HttpStatus.OK);
	}
}
